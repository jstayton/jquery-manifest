 ...
 * http://en.wikipedia.org/wiki/MIT_License
 */
(function(d,l){d.widget("mf.manifest",{options:{formatDisplay:function(a,b,c){return c?c.html():a},formatRemove:function(){return"X"},formatValue:function(a,b,c,d){return d?d.text():a},marcoPolo:false,onAdd:null,onChange:null,onHighlight:null,onHighlightRemove:null,onRemove:null,onSelect:null,onSelectRemove:null,required:false,separator:",",values:null,valuesName:null},_marcoPoloOptions:function(){var a=this,b=a.options;return{onFocus:function(){b.marcoPolo.onFocus&&b.marcoPolo.onFocus.call(this);
a._resizeInput()},onSelect:function(c,d){b.marcoPolo.onSelect&&b.marcoPolo.onSelect.call(this,c,d);a.add(c,d,true)},required:b.required}},keys:{BACKSPACE:8,DELETE:46,ENTER:13,LEFT:37,RIGHT:39},_create:function(){var a=this.options;this.$input=this.element.addClass("mf_input");this.$container=d('<div class="mf_container" />');this.$list=d('<ol class="mf_list" />');this.$measure=d('<span class="mf_measure" />');this.mpMousedown=this.mousedown=false;this.originalWidth=this.$input.css("width");a.marcoPolo&&
this._bindMarcoPolo();this._bindInput()._bindList()._bindContainer()._bindDocument();this.$input.wrap(this.$container).before(this.$list).after(this.$measure);this.$container=this.$input.parent();a.values&&this.add(a.values);this._addInputValues()._styleMeasure()._resizeInput()},_setOption:function(a,b){var c=this.$input;switch(a){case "marcoPolo":this.options.marcoPolo?b?c.marcoPolo("option",d.extend({},b,this._marcoPoloOptions())):c.marcoPolo("destroy"):b&&(this._bindMarcoPolo(d.extend({},b,this._marcoPoloOptions())),
c.marcoPolo("list").insertAfter(c.parent()));break;case "required":this.options.marcoPolo&&c.marcoPolo("option","required",b);break;case "values":this.add(b);break;case "valuesName":this.$list.find("input:hidden.mf_value").attr("name",b+"[]")}d.Widget.prototype._setOption.apply(this,arguments)},_bindMarcoPolo:function(a){var b=this,c=b.$input,h=b.options;a===l&&(a=d.extend({},h.marcoPolo,b._marcoPoloOptions()));c.marcoPolo(a);c.marcoPolo("list").bind("mousedown.manifest",function(){b.mpMousedown=
true});return b},_bindInput:function(){var a=this,b=a.$input,c=a.options;b.bind("keydown.manifest",function(c){a._resizeInput();c.which===a.keys.ENTER&&c.preventDefault();if(!b.val())switch(c.which){case a.keys.BACKSPACE:case a.keys.DELETE:c=a._selected();c.length?a.remove(c):a._selectPrev();break;case a.keys.LEFT:a._selectPrev();break;case a.keys.RIGHT:a._selectNext();break;default:a._removeSelected()}}).bind("keypress.manifest",function(d){!c.required&&d.which===c.separator.charCodeAt()&&(d.preventDefault(),
b.val()&&a.add(b.val(),null,true))}).bind("keyup.manifest",function(){a._resizeInput()}).bind("blur.manifest",function(){setTimeout(function(){a.mousedown||a._removeSelected();a.mpMousedown||(c.marcoPolo&&c.required?a._resizeInput():b.val()&&a.add(b.val(),null,true))},1)});return a},_bindList:function(){var a=this;a.$list.delegate("li","mouseover",function(){a._addHighlight(d(this))}).delegate("li","mouseout",function(){a._removeHighlight(d(this))}).delegate("li","mousedown",function(){a.mousedown=
true}).delegate("li","click",function(){a._toggleSelect(d(this))}).delegate("a.mf_remove","click",function(b){a.remove(d(this).closest("li"));b.preventDefault()});return a},_bindContainer:function(){var a=this;a.$container.bind("click.manifest",function(){a.$input.focus()});return a},_bindDocument:function(){var a=this,b=a.$input;d(document).bind("mouseup.manifest",function(c){if(a.mousedown)a.mousedown=false,d(c.target).is("li.mf_item, li.mf_item *")||a._removeSelected();if(a.mpMousedown)a.mpMousedown=
false,a.options.required?a._resizeInput():b.val()&&a.add(b.val(),null,true)});return a},container:function(){return this.$container},list:function(){return this.$list},add:function(a,b,c){for(var h=this.$input,j=this.options,a=d.isArray(a)?a:[a],f,e=d(),g=d(),i=d(),g=true,k=0;k<a.length;k++)f=a[k],e=d('<li class="mf_item" />'),g=d('<a href="#" class="mf_remove" title="Remove" />'),i=d('<input type="hidden" class="mf_value" />'),e.data("manifest",f),e.html(j.formatDisplay.call(h,f,e,b)),g.html(j.formatRemove.call(h,
g,e)),j.valuesName?i.attr("name",j.valuesName+"[]"):i.attr("name",h.attr("name")+"_values[]"),i.val(j.formatValue.call(h,f,i,e,b)),e.append(g,i),g=this._trigger("add",[f,e]),g!==false&&e.appendTo(this.$list),this._trigger("change",["add",f,e]);c&&this._clearInputValue()},_addInputValues:function(){var a=this.$input,b=a.data("values"),a=a.val(),c=[];b?c=d.isArray(b)?b:[b]:a&&(c=a.split(this.options.separator),c=d.map(c,d.trim));c.length&&this.add(c,null,true);return this},remove:function(a){var b=
this,c=d(),c=a instanceof jQuery?a:b.$list.children(a);c.each(function(){var a=d(this),c=a.data("manifest"),f=true,f=b._trigger("remove",[c,a]);f!==false&&a.remove();b._trigger("change",["remove",c,a])})},values:function(){var a=[];this.$list.find("input:hidden.mf_value").each(function(){a.push(d(this).val())});return a},destroy:function(){var a=this.values().join(this.options.separator+" ");this.options.marcoPolo&&this.$input.marcoPolo("destroy");this.$list.remove();this.$measure.remove();this.$input.unwrap().removeClass("mf_input").width(this.originalWidth).val(a);
d(document).unbind(".manifest");d.Widget.prototype.destroy.apply(this,arguments)},_styleMeasure:function(){var a=this.$input;this.$measure.css({fontFamily:a.css("font-family"),fontSize:a.css("font-size"),fontStyle:a.css("font-style"),fontVariant:a.css("font-variant"),fontWeight:a.css("font-weight"),left:-9999,letterSpacing:a.css("letter-spacing"),position:"absolute",textTransform:a.css("text-transform"),top:-9999,whiteSpace:"nowrap",width:"auto",wordSpacing:a.css("word-spacing")});return this},_measureText:function(a){var b=
this.$measure,a=a.replace(/&/g,"&amp;").replace(/\s/g,"&nbsp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");b.html(a);return b.width()},_maxInputWidth:function(){var a=this.$input;return this.$container.width()-(a.outerWidth(true)-a.width())},_resizeInput:function(){var a=this.$input,b;b=this._measureText(a.val()+"---");a.width(Math.min(b,this._maxInputWidth()));return this},_clearInputValue:function(){var a=this.$input;this.options.marcoPolo?a.marcoPolo("change",""):a.val("");this._resizeInput();return this},
_highlighted:function(){return this.$list.children("li.mf_highlighted")},_addHighlight:function(a){if(!a.length)return this;this._removeHighlighted();a.addClass("mf_highlighted");this._trigger("highlight",[a.data("marcoPolo"),a]);return this},_removeHighlight:function(a){if(!a.length)return this;a.removeClass("mf_highlighted");this._trigger("highlightRemove",[a.data("marcoPolo"),a]);return this},_removeHighlighted:function(){return this._removeHighlight(this._highlighted())},_selected:function(){return this.$list.children("li.mf_selected")},
_addSelect:function(a){if(!a.length)return this;this._removeSelected();a.addClass("mf_selected");this._trigger("select",[a.data("marcoPolo"),a]);return this},_removeSelect:function(a){if(!a.length)return this;a.removeClass("mf_selected");this._trigger("selectRemove",[a.data("marcoPolo"),a]);return this},_removeSelected:function(){return this._removeSelect(this._selected())},_toggleSelect:function(a){return a.hasClass("mf_selected")?this._removeSelect(a):this._addSelect(a)},_selectPrev:function(){var a=
this._selected(),b=d(),b=a.length?a.prev():this.$list.children(":last");b.length&&this._addSelect(b);return this},_selectNext:function(){var a=this._selected(),b=a.next();return b.length?this._addSelect(b):this._removeSelect(a)},_trigger:function(a,b){var c="on"+a.charAt(0).toUpperCase()+a.slice(1),d=this.widgetEventPrefix.toLowerCase()+a.toLowerCase(),c=this.options[c];this.element.trigger(d,b);return c&&c.apply(this.element,b)}})})(jQuery);
