 ...
 * http://en.wikipedia.org/wiki/MIT_License
 */
(function(a){a.widget("mf.manifest",{options:{formatDisplay:function(c,b,d){if(d){return d.html()}else{return c}},formatRemove:function(c,b){return"X"},formatValue:function(d,c,b,e){if(e){return e.text()}else{return d}},marcoPolo:{},onAdd:null,onHighlight:null,onHighlightRemove:null,onRemove:null,onSelect:null,onSelectRemove:null,required:false,separator:",",valuesName:null},_marcoPoloOptions:function(){var b=this,d=b.$input,c=b.options;return{onFocus:function(f,e){if(c.marcoPolo.onFocus){c.marcoPolo.onFocus.call(f,f,e)}b._resizeInput()},onSelect:function(g,h,f,e){if(c.marcoPolo.onSelect){c.marcoPolo.onSelect.call(f,g,h,f,e)}b._add(g,h)},required:c.required}},keys:{BACKSPACE:8,DELETE:46,LEFT:37,RIGHT:39},_create:function(){var b=this;b.$input=b.element.addClass("mf_input");b.$container=a('<div class="mf_container" />');b.$list=a('<ol class="mf_list" />');b.$measure=b._buildMeasure();b.mousedown=false;b.mpMousedown=false;b.originalWidth=b.$input.css("width");if(b.options.marcoPolo){b._bindMarcoPolo()}b._bindInput()._bindList()._bindContainer()._bindDocument();b.$input.wrap(b.$container).before(b.$list).after(b.$measure);b._resizeInput()},_setOption:function(c,d){var b=this,e=b.$input;switch(c){case"marcoPolo":if(b.options.marcoPolo){if(d){e.marcoPolo("option",a.extend({},d,b._marcoPoloOptions()))}else{e.marcoPolo("destroy")}}else{if(d){b._bindMarcoPolo(a.extend({},d,b._marcoPoloOptions()));e.marcoPolo("list").insertAfter(e.parent())}}break;case"required":if(b.options.marcoPolo){e.marcoPolo("option","required",d)}break;case"valuesName":b.$list.find("input:hidden.mf_value").attr("name",d+"[]");break}a.Widget.prototype._setOption.apply(b,arguments)},container:function(){return this.$container},list:function(){return this.$list},_add:function(c,e){var b=this,d=b.$input;b.add(c,e);if(b.options.marcoPolo){d.marcoPolo("change","")}else{d.val("")}b._resizeInput();return b},add:function(b,c){var h=this,d=h.$input,i=h.options,f=a('<li class="mf_item" />'),j=a('<a href="#" class="mf_remove" title="Remove" />'),e=a('<input type="hidden" class="mf_value" />'),g=true;f.data("manifest",b);f.html(i.formatDisplay.call(d,b,f,c));j.html(i.formatRemove.call(d,j,f));if(i.valuesName){e.attr("name",i.valuesName+"[]")}else{e.attr("name",d.attr("name")+"_values[]")}e.val(i.formatValue.call(d,b,e,f,c));f.append(j,e);g=h._trigger("add",[b,f]);if(g!==false){f.appendTo(h.$list)}},remove:function(b){var c=this,e=c.$input,d=a();if(b instanceof jQuery){d=b}else{d=c.$list.children(b)}d.each(function(){var g=a(this),f=true;f=c._trigger("remove",[g.data("manifest"),g]);if(f!==false){g.remove()}})},destroy:function(){var b=this;if(b.options.marcoPolo){b.$input.marcoPolo("destroy")}b.$list.remove();b.$measure.remove();b.$input.unwrap().removeClass("mf_input").width(b.originalWidth);a(document).unbind(".manifest");a.Widget.prototype.destroy.apply(b,arguments)},_trigger:function(d,f){var c=this,e="on"+d.charAt(0).toUpperCase()+d.slice(1),b=c.widgetEventPrefix.toLowerCase()+d.toLowerCase(),g=c.options[e];c.$input.trigger(b,f);return g&&g.apply(c.$input,f)},_buildMeasure:function(){var b=this.$input;return a('<measure class="mf_measure" />').css({fontFamily:b.css("fontFamily"),fontSize:b.css("fontSize"),fontWeight:b.css("fontWeight"),left:-9999,letterSpacing:b.css("letterSpacing"),position:"absolute",top:-9999,whiteSpace:"nowrap",width:"auto"})},_bindMarcoPolo:function(d){var b=this,e=b.$input,c=b.options;if(typeof d==="undefined"){d=a.extend({},c.marcoPolo,b._marcoPoloOptions())}e.marcoPolo(d);e.marcoPolo("list").bind("mousedown.manifest",function(){if(!c.required){b.mpMousedown=true}});return b},_bindInput:function(){var b=this,d=b.$input,c=b.options;d.bind("keydown.manifest",function(f){b._resizeInput();if(d.val()){return}switch(f.which){case b.keys.BACKSPACE:case b.keys.DELETE:var e=b._selected();if(e.length){b.remove(e)}else{b._selectPrev()}break;case b.keys.LEFT:b._selectPrev();break;case b.keys.RIGHT:b._selectNext();break;default:b._removeSelected();break}}).bind("keypress.manifest",function(e){if(!c.required&&e.which===c.separator.charCodeAt()){e.preventDefault();if(d.val()){b._add(d.val(),null)}}}).bind("keyup.manifest",function(e){b._resizeInput()}).bind("blur.manifest",function(){setTimeout(function(){if(!b.mousedown){b._removeSelected()}if(!b.mpMousedown&&!c.required&&d.val()){b._add(d.val(),null)}},1)});return b},_bindList:function(){var b=this;b.$list.delegate("li","mouseover",function(){b._addHighlight(a(this))}).delegate("li","mouseout",function(){b._removeHighlight(a(this))}).delegate("li","mousedown",function(){b.mousedown=true}).delegate("li","click",function(){b._toggleSelect(a(this))}).delegate("a.mf_remove","click",function(c){b.remove(a(this).closest("li"));c.preventDefault()});return b},_bindDocument:function(){var b=this,c=b.$input;a(document).bind("mouseup.manifest",function(d){if(b.mousedown&&!a(d.target).is("li.mf_item, li.mf_item *")){b.mousedown=false;b._removeSelected()}if(b.mpMousedown&&!b.options.required){b.mpMousedown=false;if(c.val()){b._add(c.val(),null)}}});return b},_bindContainer:function(){var b=this;b.$container.bind("click.manifest",function(){b.$input.focus()});return b},_highlighted:function(){return this.$list.children("li.mf_highlighted")},_addHighlight:function(c){var b=this;if(!c.length){return b}b._removeHighlighted();c.addClass("mf_highlighted");b._trigger("highlight",[c.data("marcoPolo"),c]);return b},_removeHighlight:function(c){var b=this;if(!c.length){return b}c.removeClass("mf_highlighted");b._trigger("highlightRemove",[c.data("marcoPolo"),c]);return b},_removeHighlighted:function(){return this._removeHighlight(this._highlighted())},_selected:function(){return this.$list.children("li.mf_selected")},_addSelect:function(c){var b=this;if(!c.length){return b}b._removeSelected();c.addClass("mf_selected");b._trigger("select",[c.data("marcoPolo"),c]);return b},_removeSelect:function(c){var b=this;if(!c.length){return b}c.removeClass("mf_selected");b._trigger("selectRemove",[c.data("marcoPolo"),c]);return b},_removeSelected:function(){return this._removeSelect(this._selected())},_toggleSelect:function(b){if(b.hasClass("mf_selected")){return this._removeSelect(b)}else{return this._addSelect(b)}},_selectPrev:function(){var c=this,d=c._selected(),b=a();if(d.length){b=d.prev()}else{b=c.$list.children(":last")}if(b.length){c._addSelect(b)}return c},_selectNext:function(){var c=this,d=c._selected(),b=d.next();if(b.length){return c._addSelect(b)}else{return c._removeSelect(d)}},_resizeInput:function(){var d=this,e=d.$input,c=d.$measure,b=e.val().replace(/&/g,"&amp;").replace(/\s/g,"&nbsp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");c.html(b+"---");e.width(c.width());return d}})})(jQuery);
